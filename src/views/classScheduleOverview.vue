<template>
  <div class="classScheduleOverview">
    ...
    <div class="content" v-if="activeIndex == 0" style="overflow-y: auto">
      ...
      <!-- 导出教室总表 -->
      <Modal v-model="exportModel" title="导出教室总表">
        <p>导出教室总表</p>
        <div slot="footer" style="text-align: center">
          <Button @click="exportModelCancel">取消</Button>
          <Button type="primary" @click="exportModelOk">导出课表</Button>
        </div>
      </Modal>
    </div>
...
export default {
  data() {
    return {
      ...
      // 周次
      weeklys: [
        { weekName: "第1周", beginDate: "2021-06-27", endDate: "2021-07-03" },
        { weekName: "第2周", beginDate: "2021-07-03", endDate: "2021-07-10" },
        { weekName: "第3周", beginDate: "2021-07-10", endDate: "2021-07-17" },
        { weekName: "第4周", beginDate: "2021-07-17", endDate: "2021-07-24" },
        { weekName: "第5周", beginDate: "2021-07-24", endDate: "2021-04-31" },
      ],
      // 楼栋信息
      buildings: [],
  ...
  methods: {
    exportData() {
      ...
      //   filename: "教室课表总表",
      //   // columns: this.classroomScheduleColumns,
      //   // data: this.classroomScheduleData
      // });
    },
    // 教室课表的多选框 与 课表的对应
    setWeekClassroomSchedule() {
      ...
      this.search.classroomWeek.forEach((item, i) => {
        switch (item) {
          case "星期一":
            week = "week1";
            this.setCourse(item, i, week, item);
            break;
          case "星期二":
            week = "week2";
            this.setCourse(item, i, week, item);
    ...
    showClassroomModel(row, weekStr) {
      this.title = `添加上课信息 ( ${row.row.className} ${this.search.classroomWeekly} ${weekStr} 第${row.column.title}节)`;
                            ...
                            "DropdownItem",
                            {
                              nativeOn: {
                                click: () => {
                                  let adjustCourseObj =
                                    params.row[params.column.key];
                                  this.removeCourse(adjustCourseObj);
                                },
                              },
                            },
                            "移除"
                        ...
                        "div",
                        {
                          ...
                          on: {
                            click: (e) => {
                              ...
                              let assemble = teacherScheduleItems1.concat(
                                teacherScheduleItems2
                              );
                              assemble.forEach((e) => {
    ...
    sendAdjustCourseInfo() {
      ...
      // 后端
      console.log(this.adjustCourseInfoData);
      ...
      // 或者发两个课程安排的ID？
      let data = {
        adjustCourseObj: {}, //调课对象
        adjustCourseToObj: {}, //调至对象
      };

      teacherScheduleItems1.forEach((e) => {
        if (
          e.classroom == adjustCourseObj.classroom &&
          e.week == adjustCourseObj.week &&
          e.section == adjustCourseObj.section
        ) {
          if (!this.conflict) {
            // 无冲突
            e.week = adjustCourseToObj.week;
            e.section = adjustCourseToObj.section;
          } else {
            // 有冲突并且不是点击了当前老师的课表中本来就有课程的位置
            if (adjustCourseToObj.id) {
              let toWeekAndSection = {
                week: adjustCourseObj.week,
                section: adjustCourseObj.section,
              };
              e.week = adjustCourseToObj.week;
              e.section = adjustCourseToObj.section;
              teacherScheduleItems1.forEach((item) => {
                if (
                  item.teachName == adjustCourseToObj.teachName &&
                  item.classroom == adjustCourseToObj.classroom &&
                  item.week == adjustCourseToObj.week &&
                  item.section == adjustCourseToObj.section
                ) {
                  item.week = toWeekAndSection.week;
                  item.section = toWeekAndSection.section;
                }
              });
            } else {
              this.$Message.error(adjustCourseToObj.msg);
            }
          }
        }
      });
    ...
    sendAdjustToForm() {
      ...
      console.log(this.adjustCourseData.section[0]);

      let data = {
        classNameId: this.adjustCourseData.classNameID,
        courseId: this.adjustCourseData.courseId,
        sectionID: this.adjustCourseData.section[0],
        teachName: this.adjustCourseData.teachName,
        week: this.adjustCourseData.week,
        weekly: this.adjustCourseData.weekly,
      ...
      this.teacherScheduleItems.push(data);
      this.adjustTo = true;
      console.log(data);
      this.getSchedule();
    },
    ...
    // https://blog.csdn.net/qq_38523017/article/details/110181060
    // 行、列合并
    handleSpan({ row, column, rowIndex, columnIndex }) {
      if (this.activeIndex == 0) {
        // 列合并 开始列是多少？合并多少列？
        if (rowIndex == 2 && columnIndex === row.preIndex * 1) {
          return [1, row.mergeRow * 1];
        } else if (
          rowIndex == 2 &&
          columnIndex > row.preIndex * 1 &&
          columnIndex < row.preIndex * 1 + row.mergeRow * 1
        ) {
          return [0, 0];
        }
      } else {
    ...
    assembleData(data) {
      if (this.activeIndex === 0) {
        ...
        data.forEach((e, i) => {
          ...
          if (e.week3) {
            let week3Arr = Object.keys(e.week3); //代表了有课的节次
            // console.log(week3Arr);

            week3Arr.forEach((section, index) => {
              // console.log(section, week3Arr[index + 1]);

              // console.log(e.week3[(section*1+1)]);
              // console.log(e.week3[3]);

              if (e.week3[section * 1 + 1] && e.week3[section].already != 1) {
                if (preIndex == -1 || section != week3Arr[index - 1]) {
                  e.preIndex = section;
                }
                preIndex = index;

                if (e.week3Str[`sectionStr${section}`]) {
                  e.mergeRow = 1;
                  for (let a = index + 1; a < week3Arr.length; a++) {
                    // 判断 当前项的信息 == 下一项的信息
                    if (
                      e.week3Str[`sectionStr${section}`] ==
                      e.week3Str[`sectionStr${week3Arr[index + 1]}`]
                    ) {
                      e.mergeRow++;

                      e.week3[week3Arr[index + 1]].already = 1;
                    }
                  }
                }
              }
    ...
    searchClassSchedule() {
      ...
      if (this.search.className) {
        let a = []; //中间值
        this.classScheduleItems.forEach((e) => {
          if (e.className === this.search.className) {
            a.push(e);
          }
        });
        this.classScheduleItems = a;
      } else if (
        this.search.weekly &&
        (this.search.className == "" || this.search.className == undefined)
      ) {
        let a = []; //中间值
        this.classScheduleItems.forEach((e) => {
          if (e.weekly.indexOf(this.search.weekly * 1) != -1) {
            a.push(e);
          }
        });
        this.classScheduleItems = a;
      ...
      if (this.search.teacher) {
        if (this.search.teacher == "李四") {
          this.teacherScheduleItems = teacherScheduleItems2;
        } else {
          this.teacherScheduleItems = teacherScheduleItems1;
        }
      }

      // this.search.buildingId
      if (this.search.buildingId) {
        console.log(this.search.buildingId);
      }
    ...
    // 获得学期
     getTerms() {
      ...
      this.axios
        ...
        .then((resp) => {
          if (resp.data.state) {
            that.compareDate(that.curTime, null, resp.data.termList);
            that.getWeekly();
          } else {
    ...
    compareDate(date, weeklys, curTerm) {
      ...
      if (weeklys) {
        weeklys.forEach((item) => {
          ...
          if (start < stamp && stamp < end) {
            this.search.classroomWeekly = item.weekName;
            this.search.teacherWeekly = item.weekName;
            this.search.weekly = item.weekName;
